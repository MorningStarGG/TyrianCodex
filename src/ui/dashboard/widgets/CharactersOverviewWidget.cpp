...
void DashW::CharactersOverview(App& app, float w)
{
    ...
    for (const AccountData::AcctChar& c : m.characters)
    {
        ...
        if (DashUtil::Narrow(w))
        {
            ...
            ImGui::Dummy(ImVec2(w, 31.f));
            Gw2Ui::LabelDL(dl, ImVec2(p.x, p.y),         ImVec2(p.x + w, p.y + 17.f), c.name.c_str(), Gw2Ui::HAlign::Left, Gw2Ui::VAlign::Middle, nameCol, false, nullptr, 14.f);
            Gw2Ui::LabelDL(dl, ImVec2(p.x, p.y + 16.f),  ImVec2(p.x + w, p.y + 31.f), meta,           Gw2Ui::HAlign::Left, Gw2Ui::VAlign::Middle, metaCol, false, nullptr, 14.f);
        }
        else
        {
            // wide: name + right-aligned meta on one line; clip the name to the MEASURED meta column (not a fixed 100)
            const float metaW = Gw2Ui::MeasureWidth(meta, 14.f) + 10.f;
            ImGui::Dummy(ImVec2(w, 19.f));
            const Gw2Ui::RowHotspot row{ p, w, 19.f, false, false };
